...
The **napari viewer** window will appear as shown below.
...
When loaded, the image appears as a new layer named after the file,  
and an **ROIs** layer is automatically created on top of it.
...
1. Click **Add Layer** to create a *Scale Measurement* shapes layer.
2. Set the **mode** to `px/cm`.
3. Zoom in on the scale bar and confirm that **Add lines** 

{width="24"} is selected.
...
*Binarization* converts a grayscale image into two tones — typically white (object) and black (background) —  
based on pixel intensity.  
This enhances contrast and allows precise contour extraction.
...
-   Segment Anything Model 2 (SAM2) [@ravi2024sam2segmentimages]: 
    A deep-learning segmentation model developed by Meta.
...
| Tool | Use |
| --- | --- |
| **Paint brush** | Fill missing or small areas |
| **Polygon tool** | Add or correct large regions |
| **Label eraser** | Remove unwanted areas |
